...
impl ScalarQuantizer{
    ...
    pub fn merge(&mut self, matrix:Vec<Vec<f64>>){
        ...
        for i in 0..self.levels{
            self.quantiles.push(self.t_digest.estimate_quantile((i as f64)/(self.levels.clone() as f64)));
        }
    ...
    pub fn quantize_vectors(&self, vecs: Vec<Vec<f64>>) -> Vec<Vec<usize>> {
        ...
        quantized.chunks(self.dim.clone()).map(|chunk| chunk.to_vec()).collect()
    ...
    pub fn quantize(&self, v: &[f64])->Vec<usize>{
        v.iter().map(|value| self.__quantize_scalar(value)).collect()
    }
